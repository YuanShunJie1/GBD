...
class UBDDefense(object):
    ...
    def backdoor_check(self, caps, cacs, active_id=0):
        batch_size  = len(caps)
    ...
    def visualize_single_caps(self, caps, file_name, who, backdoor_indices): 
        caps = np.array(caps)
        ...
        # file_path_svg = f'{file_name}_cap.svg'
    ...
    def visualize_single_cacs(self, cacs, file_name, who, backdoor_indices):
        cacs = np.array(cacs)
        ...
        plt.scatter(clean_indices, cacs[clean_indices], c='#5e9e6e', marker='o', label='Clean')
        plt.scatter(backdoor_indices, cacs[backdoor_indices], c='#b55d60', marker='o', label='Backdoor')
        ...
        # file_path_svg = f'{file_name}_cac.svg'
        # plt.savefig(file_path_svg, bbox_inches='tight')
        # drawing = svg2rlg(file_path_svg)
        # file_path_pdf = f'{file_name}_cac.pdf'
        # renderPDF.drawToFile(drawing, file_path_pdf)
    ...
    def detect(self, image_path, idx, active_model, embeddings, labels, repeat_times, ground_truth=[], poison_indices=[]):
        ...
        # # 只画前5个Batch
        if idx < 5:
            # self.visualize(caps, cacs, f'{image_path}/{idx}', 1, poison_indices)
            self.visualize_single_caps(caps, f'{image_path}/{idx}', 1, poison_indices)
            self.visualize_single_cacs(cacs, f'{image_path}/{idx}', 1, poison_indices)

        b_idx, c_idx, indicator = self.backdoor_check(caps, cacs, active_id=0)
    ...
    #         temp_pred = top_model(None, None, agged_inputs=temp, agged=True)

    #         loss = F.cross_entropy(temp_pred, labels_tensor[i].unsqueeze(0))
    ...
    #     backdoor_indices = np.where(indicator == 1)[0]

    #     plt.clf()

    #     plt.figure(figsize=(8, 6))
    #     plt.scatter(caps[clean_indices], cacs[clean_indices], alpha=0.7, c='blue', edgecolors='k', label='Clean')
    #     plt.scatter(caps[backdoor_indices], cacs[backdoor_indices], alpha=0.9, c='red', edgecolors='k', label='Backdoor')
