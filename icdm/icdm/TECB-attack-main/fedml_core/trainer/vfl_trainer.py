...
class VFLTrainer(ModelTrainer):
    ...
    def extract_features(self, train_data, device, args):
        ...
        with torch.no_grad():
            for step, (trn_X, trn_y) in enumerate(train_data):
                if args.dataset in ['CIFAR10', 'CIFAR100', 'CINIC10L', 'MNIST', 'FashionMNIST']:
    ...
    def test_cluster(self, test_data, criterion, device, args):
        ...
        print('C-Test: Average loss: {:.4f}, Top-1: {}/{} ({:.2f}%)\n'.format(test_loss, correct, total, top1_acc))
    ...
    def test_mul(self, test_data, criterion, device, args):
        ...
        # print('C-Test: Average loss: {:.2f}, Top-1: {}/{} ({:.2f}%)'.format(test_loss, correct, total, top1_acc))
    ...
    def test_backdoor_mul(self, test_data, criterion, device, args, delta, poison_target_label):
        ...
        # print('B-Test: Average loss: {:.2f}, ASR Top-1: {}/{} ({:.2f})%'.format(test_loss, correct, total, top1_acc))

        return test_loss, top1_acc, top5_acc



    def test_backdoor_detection_mul(self, test_data, criterion, device, args, delta, poison_target_label, udb, poison_rate=0.5, image_path=''):
        ...
        for batch_idx, (trn_X, trn_y, indices) in enumerate(test_data):
            ...
            poison_size = int(poison_rate*batch_size)
            ...
            # temp = Xb[poison_indices] + delta
            Xb[poison_indices] += delta
            ...
            embeddings = [output_tensor_bottom_model_a, output_tensor_bottom_model_b]
            # _, _, detection = 
            acc = udb.detect(image_path, batch_idx, model_list[2], embeddings, prediction, args.repeat, ground_truth, poison_indices.tolist())
    ...
    def test_backdoor_detection_mul_baseline(self, test_data, criterion, device, args, delta, poison_target_label, udb, poison_rate=0.5, image_path=''):
        ...
        for batch_idx, (trn_X, trn_y, indices) in enumerate(test_data):
            ...
            embeddings = np.array(output_tensor_bottom_model_b.detach().cpu())
    ...
    def train_narcissus(self, train_data, criterion, bottom_criterion, optimizer_list, device, args, delta,
        ...
        return sum(epoch_loss)/len(epoch_loss), delta
