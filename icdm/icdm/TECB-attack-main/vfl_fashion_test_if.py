...
from fedml_core.data_preprocessing.cifar10.dataset import IndexedMNIST, IndexedFashionMNIST
from fedml_core.model.baseline.vfl_models import BottomModelForMnist, TopModelForMnist
...
def main(device, args):
    ...
    trainset = IndexedFashionMNIST(root='./data', train=True, download=True, transform=train_transform)
    testset = IndexedFashionMNIST(root='./data', train=False, download=True, transform=train_transform)

    # CIFAR-10 类别标签（以类别名称的列表形式给出）
    # classes = ('0', '1', 'd', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')
    ...
    # build model
    model_list = []
    model_list.append(BottomModelForMnist())
    model_list.append(BottomModelForMnist())
    model_list.append(TopModelForMnist())

    criterion = nn.CrossEntropyLoss().to(device)
    bottom_criterion = keep_predict_loss

    # 加载预训练模型
    save_model_dir = args.save + "/saved_models"
    ...
    test_loss, test_asr_acc, _ = vfltrainer.test_backdoor_mul(non_target_queue, criterion, device, args,delta, target_label)
    


    device = next(model_list[0].parameters()).device
    
    udb = UBDDefense(device=device)
    
    if not os.path.exists(f'/home/shunjie/codes/vfl_backdoor_defense/icdm/TECB-attack-main/images/image_fashionmnist_{args.poisoning_rate}_{args.repeat}'):
        os.makedirs(f'/home/shunjie/codes/vfl_backdoor_defense/icdm/TECB-attack-main/images/image_fashionmnist_{args.poisoning_rate}_{args.repeat}')
    
    avd = vfltrainer.test_backdoor_detection_mul_baseline(non_target_queue, criterion, device, args, delta, 0, udb, poison_rate=args.poisoning_rate, image_path=f'/home/shunjie/codes/vfl_backdoor_defense/icdm/TECB-attack-main/images/image_fashionmnist_{args.poisoning_rate}_{args.repeat}')
    
    print("fashionmnist", args.poisoning_rate, args.repeat, "top1_acc: ", top1_acc, "top5_acc: ", top5_acc, "test_asr_acc: ", test_asr_acc, "backdoor_detection_acc", avd)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', default='MNIST', type=str,
                        ...
                        choices=['CIFAR10', 'CIFAR100', 'TinyImageNet', 'CINIC10L', 'Yahoo', 'Criteo', 'BCW','MNIST','FashionMNist'])
    parser.add_argument('--name', type=str, default='vfl_mnist', help='experiment name')
    ...
    parser.add_argument('--save', default='./model/FashionMNIST/baseline', type=str,
                        metavar='PATH',
    ...
    parser.add_argument('--target_class', type=str, default='0', help='backdoor target class')
    ...
    # config file
    parser.add_argument('--c', type=str, default='configs/base/fashion_bestattack.yml', help='config file')
