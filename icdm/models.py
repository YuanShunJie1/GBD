...
class MLPNet(nn.Module):
    def __init__(self,linear_size,emb_size):
        super(MLPNet, self).__init__()
        self.fc1 = nn.Linear(linear_size, 256)
...
class FC1_passive(nn.Module):
    ...
    def __init__(self, num_passive, linear_size):
        ...
        # self.embeddings = MLPNet(linear_size,emb_size)
        # print("Passive Model", self.embeddings)
...
class ResNet(nn.Module):
    ...
    '''
    def __init__(self, block, layers, num_classes, num_passive, device, **kwargs):
        super(ResNet, self).__init__()
        self.num_passive = num_passive

        # 将 passive 改为 nn.ModuleList
        self.passive = nn.ModuleList([
            ResNet_passive(block, layers, num_classes=128).to(device) for _ in range(num_passive)
        ])
        
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.active = ResNet_active(input_size=128 * num_passive, num_classes=num_classes).to(device)
    ...
    def todevice(self, device):
        ...
        for mm in self.passive:
            mm = mm.to(device)



def resnet18(num_classes=10, num_passive=2, device=None):
    return ResNet(block=ResidualBlock, layers=[2, 2, 2, 2], num_classes=num_classes, num_passive=num_passive, device=device)



# def resnet():
#     ResNet(block=ResidualBlock, layers=[2, 2, 2, 2], num_classes=10)
#     return 
...
#     'cinic10': partial(ResNet, block=ResidualBlock, layers=[2, 2, 2, 2], num_classes=10)
# }




# def forward(self, x, attacker_id=0, backdoor=False, trigger=None):
#     x = list(x)
#     emb = []
#     for i in range(self.num_passive):
#         if backdoor:
