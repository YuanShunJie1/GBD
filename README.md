...
icdm和tifs这两个文件夹分别对应着TECB后门攻击和BASL后门攻击。我们分别在这两个攻击下实现了我们后门检测方法GBD。我们以检测BASL攻击为例子。在tifs目录中，main.py代表运行BASL后门攻击，test.py代表利用我们的方案进行后门检测，attackers目录下的ubd.py文件表示我们方案。首先你需要运行main.py对VFL发起后门攻击，其次运行test.py就可以在推理阶段实现后门检测。
