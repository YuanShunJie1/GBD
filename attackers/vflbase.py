...
class BaseVFL(object):
    def __init__(self, args, entire_model, train_loader, test_loader, device):  # passive_model, active_model
        # setup arguments
        self.args = args
        ...
        #     self.optimizer_entire = torch.optim.Adam(self.model.parameters(), lr=args.lr_active)
        #     self.optimizer_active = torch.optim.Adam(self.model.active.parameters(), lr=args.lr_active)
        #     self.optimizer_passive = []
        #     for i in range(args.num_passive):
        #         lr = args.lr_attack if i in args.attack_id else args.lr_passive
        #         self.optimizer_passive.append(torch.optim.Adam(self.model.passive[i].parameters(), lr=lr))
        # else:
        # self.optimizer_entire = torch.optim.SGD(self.model.parameters(), lr=args.lr_active)
        
        if self.args.dataset == 'mnist' or self.args.dataset == 'fashionmnist' or self.args.dataset == 'cdc' or self.args.dataset == 'aids':
            self.optimizer_entire = torch.optim.Adam(self.model.parameters(), lr=args.lr_active)
            self.scheduler_entire = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_entire, milestones=args.schedule, gamma=0.1)
        else:
            self.optimizer_entire = torch.optim.SGD(self.model.parameters(), lr=args.lr_active, momentum=0.9, weight_decay=5e-4)
            self.scheduler_entire = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_entire, milestones=args.schedule, gamma=0.1)
        ...
        # self.scheduler_active = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_active, milestones=args.schedule, gamma=0.1)
        # self.scheduler_passive = []
        # for i in range(args.num_passive):
        #     self.scheduler_passive.append(torch.optim.lr_scheduler.MultiStepLR(self.optimizer_passive[i], milestones=args.schedule, gamma=0.1))
    ...
    #             data, labels, index = batch_data  # len(data) = batch_size 128
    #             # for mnist and fashionmnist: torch.Size([128, 1, 28, 28])
    #             # for cifar10: torch.Size([128, 3, 32, 32])
    #             if self.args.dataset == "criteo":
    #                 self.train_dataset.append([[temp for temp in torch.chunk(data, self.args.num_passive, dim=1)], labels, index])
    #             else:
    #                 self.train_dataset.append([[temp for temp in torch.chunk(data, self.args.num_passive, dim=3)], labels, index])
    ...
    #             if self.args.dataset == "criteo":
    #                 self.test_dataset.append([[temp for temp in torch.chunk(data, self.args.num_passive, dim=1)], labels, index])
    #             else:
    #                 self.test_dataset.append([[temp for temp in torch.chunk(data, self.args.num_passive, dim=3)], labels, index])
    ...
    #             self.train_dataset.append([data_list, labels, index])
    #             self.train_dataset_len += len(data)
    ...
    #             for i in range(self.args.num_passive):
    #                 data_list.append(data.index_select(3, torch.tensor(sample_list[i])))
    #             self.test_dataset.append([data_list, labels, index])
    #             self.test_dataset_len += len(data)
    ...
    #             data, labels, index = batch_data  # data is tuple, len(data[0]) = batch_size
    ...
    #             train_loss += F.cross_entropy(pred, labels).item()
    ...
    #             passive_emb.append(data[passive_id].cpu().reshape(data[passive_id].shape[0], -1).detach().numpy())  # KMeans expected dim <= 2.
